      ...
      <div className="w-[90%] max-w-sm p-4 bg-gray-400 flex-col flex items-center gap-3 rounded-xl shadow-slate-600 shadow-lg">
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          <p className="text-xs md:text-sm text-center">
            {isLogin? "Don't have an account?" : "Already have account?"} <span onClick={toggleLogin} className="text-purple-600 font-semibold cursor-pointer underline hover:text-purple-700">{isLogin ? "Sign Up" : "Sign In"}</span>
          </p>
        </div>
        ...
        <div className="w-full flex flex-col gap-3">
          <div className="w-full flex items-center bg-gray-300 p-2 rounded-xl gap-2">
            ...
            <div className="flex-1">
              <InputBox type={showPassword ? "password" : "text"} placeholder="example@gmail.com" />
            </div>
            <span className="text-gray-800 pt-1 cursor-pointer">
              {showPassword ? <OpeneyeIcon onClick={togglePassword} /> : <ClosedeyeIcon onClick={togglePassword}/>}
            </span>
          </div>
        </div>
        <Button varient="primary" text={isLogin? "Sign In" : "Sign Up"} fullWidth={true} />
